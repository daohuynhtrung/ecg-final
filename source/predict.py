...
def load_model_to_estimate(modelName):
    ...
    for i in y_pred:
        if i==1:
            sick_count+=1
    print('Phat hien ',sick_count,' bat thuong trong tong so ',len(Y_test))
    print('Ket luan:')
    if((sick_count/len(Y_test))<0.1):
        print('Tim hoat dong binh thuong')
    else:
        print('Co dau hieu bat thuong tim mach')
